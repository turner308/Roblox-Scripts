...
function Flux:Window(text, bottom, mainclr, toclose)
	...
	function Tabs:Tab(text,ico)
		...
		function ContainerContent:ToggleDropdown(text,list,callback)
			local toggles = {}
			local DropFunc = {}
			local Selected = text
			local FrameSize = 43
			...
			function DropFunc:Add(addtext, boolean)
				boolean = boolean or false
				toggles[addtext] = boolean
				ItemCount = ItemCount + 1
				...
				Item.MouseButton1Click:Connect(function()
					toggles[addtext] = toggle
					pcall(callback, toggles)
					if toggle then
						TweenService:Create(
							Item,
							TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
