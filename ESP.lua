...
RunService.RenderStepped:Connect(function()
	...
	for _, Player in Players:GetPlayers() do
		...
		-- 2D Box
		Im.Draw2DBox(Character, Color, 1)

		-- 3D Box
		Im.Draw3DBox(Character, Color, 1)

		-- Name
		local ScreenPos, Visible = workspace.CurrentCamera:WorldToViewportPoint(Head.Position + Vector3.new(0, 0.6, 0))
		if Visible then
			Im.Text(
				ScreenPos.X,
				ScreenPos.Y,
		...
		local DistText = string.format("[%dm]", Distance)

		local DistPos, DistVisible = workspace.CurrentCamera:WorldToViewportPoint(Root.Position - Vector3.new(0, 2.8, 0))
		if DistVisible then
			Im.Text(
				DistPos.X,
				DistPos.Y,
